import { View, Text, ScrollView, Image, Pressable, StatusBar, BackHandler,AppState } from 'react-native'
...
let userInfo, allContestFees, prevContests,selectedUserMatch={},selectedUserContest={};
...
const getStatus = async(event) =>{
     ...
     if(route.name == 'FindOpponent' && stat == "false" && ds.matchId == selectedUserMatch.matchId && ds.contestId == selectedUserContest.contestId && (ds.userOne == userInfo.userId || ds.userTwo == userInfo.userId)){
...
const getDownStatus = async(event) =>{
    ...
    if(route.name == 'FindOpponent' && stat == "false" && ds.matchId == selectedUserMatch.matchId && ((ds.userOne == userInfo.userId && ds.userOneContId == selectedUserContest.contestId) || (ds.userTwo == userInfo.userId && ds.userTwoContId == selectedUserContest.contestId)) ){
        AsyncStorage.setItem("Opn", "true");
        setopEvent(true);
        UserStore.setopponentSocketData(JSON.parse(event.data));
        navigation.navigate('GameRoom',{source : "FindOpponent"});
...
const getDownGradeList = async(event) =>{
    ...
    if(route.name == 'FindOpponent' && ds.matchId == selectedUserMatch.matchId){
        const f_dgrade1 = (ds.options).filter(val => !prevContests.includes(val));
        const f_dgrade2 = f_dgrade1.filter(item => Number(item) < Number(selectedUserContest.contestId));
 ...
 const getData = async() =>{
        ...
        Functions.getInstance().checkInternetConnectivity().then((state)=>{
            if(state == true){
                ...
                Services.getInstance().findAnOpponent(obj, userInfo.userId, selectedUserMatch.matchId, selectedUserContest.contestId, userInfo.accesToken).then((result)=>{
                    console.log("Opponent Data : ", JSON.stringify(result));  
                })
                ...
                Services.getInstance().getAllDownGrades(obj, userInfo.userId, selectedUserMatch.matchId, userInfo.accesToken).then((result)=>{
                    if(result.status == 200){
                        setdownGrade(result.msg);
                        const f_dgrade1 = (result.msg).filter(val => !prevContests.includes(val));
                        const f_dgrade2 = f_dgrade1.filter(item => Number(item) < Number(selectedUserContest.contestId));
                        filterContestPrices(allContestFees, f_dgrade2); 
                        hideProgress();
                    }
    ...
    const onYes = () =>{
        ...
        Services.getInstance().updateDownGrades(obj, userInfo.userId, selectedUserMatch.matchId, selectedUserContest.contestId, userInfo.accesToken).then((result)=>{
            if(result.status == 200){
                setAlert(false);
                hideProgress();
                let date = new Date();
        ...
        return(
            <View style={[styles.opponent, styles.opmn, {marginBottom: 30,}]}>
                    <View style={styles.cntstmain}>
                        <Text style={[styles.chstxt, styles.slctd]}>Opponent is available below</Text>
                        {currentFees && currentFees.length > 0 ? currentFees.map((data, index)=>{
                            return(
                                    <Pressable onPress={()=>switchContest(data)} key={index} style={{marginBottom : 10}}>
                                        <View style={styles.opslavlbl}>
                                            <View style={styles.opslblw}>
                                                <Text style={styles.opslfee}>Each Run</Text>
                                                <Text style={styles.opslrepe}>{`₹${MatchType[(data.contType)]}`}</Text>
                                            </View>

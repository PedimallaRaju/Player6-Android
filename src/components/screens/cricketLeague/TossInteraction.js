...
const TossInteraction = ({navigation,showProgress,hideProgress,props}) => {
            ...
            if(state == true){
                ...
                Services.getInstance().selectedGameDetails(obj, userInfo.userId, gameDetails.gameId, userInfo.accesToken).then((result)=>{
                    if(result.status == 200){
                        ...
                        setTimeout(()=>{
                            hideProgress();
                            if(result.data.tossWinner == userInfo.userId){
                                navigation.navigate('TossWon',{source : "BettingAddMoney"});
                            }
                            else{
                                navigation.navigate('TossLoss',{source : "BettingAddMoney"});
                            }
...
export default WithProgress(inject('UserStore')(observer(TossInteraction)))

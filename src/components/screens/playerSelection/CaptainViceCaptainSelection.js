...
import {PlusIcon,Player1, Player2, Player3, Player4, Player5, Player6, Player7, India, England, SortIcon } from '../../../assets';
...
let userInfo, myPic, userPF, interval, timeOut, gameDetails={},loading=false;
const CaptainViceCaptainSelection = ({navigation, showProgress, hideProgress}) => {
...
useEffect(() => {
  ...
  const saveCaptainAndVC = () =>{
    if(routes.name == 'CaptainViceCaptainSelection'){
      ...
      console.log(obj);
      Services.getInstance().selectCaptainAndVC(obj,userInfo.userId,gameDetails.gameId,userInfo.accesToken).then((result)=>{
        console.log(result);
        if(result.status == 200){
          hideProgress();
          Functions.getInstance().Toast("success", "Team details have been updated");
        ...
        <View style={styles.plyrmain}>
            <View style={styles.plyrinfo}>
                ...
                <View style={[styles.plyrprflinfo, styles.srtby]}>
                    ...
                    {item.imageId == "" ? <Image style={styles.tmsdimg} source={require('../../../assets/images/dummy.png')}/>
                      :
                      <Image style={styles.tmsdimg} source={{ uri: `https://cdn.sportmonks.com/images/cricket/players/${item.imageId}` }} />
                    }
                    </View>
                    <View>
                    <Text
                ...
                <View style={[styles.srtby, styles.rnoptn]}>
                    ...
                    <View style={[styles.srtby, styles.rnoptn, styles.cvcslct]}>
                        ...
                        <View
                            style={[
                            styles.cslct,
                            (item.isCap == "1" && isOpponent === false) || (selectedCaptainId == item.playerId && isOpponent === false)
                                ? { backgroundColor: secondary }
                                : 
                              (isOpponent === true && item.isCap == "1") || (selectedCaptainId == item.playerId && isOpponent === true)
                        ...
                        <View
                            style={[
                            styles.cslct,
                            (item.isvCap == "1" && isOpponent === false) || (selectedViceCaptainId == item.playerId && isOpponent === false)
                                ? { backgroundColor: secondary }
                                : 
                              (isOpponent === true && item.isvCap == "1") || (selectedViceCaptainId == item.playerId && isOpponent === true)
                                ? { backgroundColor: '#e79013' }
                                : { backgroundColor: '#606060' },
                            ]}>
                            <Text
      ...
      <ScrollView style={styles.scrollView} keyboardShouldPersistTaps='always'>
        <View style={{backgroundColor : "#111",marginTop : 60}}>
                    <View style={[styles.headerContainer]}>
                        <Text style={[styles.headingdText]}>{userPF ? userPF?.name : ""} Team Selection</Text>
...
export default WithProgress(inject('UserStore')(observer(CaptainViceCaptainSelection)));
